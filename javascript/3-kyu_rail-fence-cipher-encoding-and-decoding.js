...
//  until you reach the top rail. Continue until you reach the end of the string.
...
const encodeRailFenceCipher = (str, n) => {
    let encoded = ""
    let ctr = 0
    let direction = true
    const levels = {}
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        // OBJECT CREATION
        levels[ctr] !== undefined ? null : levels[ctr] = ""
        levels[ctr] += str[i]
...
const decodeRailFenceCipher = (str, n) => {
    ...
    // JOIN ALL THE OBJ INDEX IN SINGLE ARR
    for (let item in levels) {
        encodedIndex = encodedIndex.concat(levels[item])
    }

    //console.log(encodedIndex.length, strArr.length)

    // CREATE CACHE
    for (let i = 0; i < encodedIndex.length; i++) {
        cache.push(null)
    }
